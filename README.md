...
A API (`apps/api`) agora persiste `Empresa` → `UnidadeNegocio` → `Lote` via Prisma e expõe um endpoint que roda o `MotorPostura` de verdade: `POST /lotes/:id/avancar-dia`. **Isso ainda não foi testado contra um Postgres real** (o ambiente de desenvolvimento atual não tem Docker/Postgres instalado) — o que foi validado sem banco: schema Prisma (`prisma validate`/`generate`), build do NestJS, boot da aplicação com `/health` respondendo mesmo sem banco (`PrismaService` conecta sob demanda, não no boot — um endpoint que precisa do banco falha com 500 claro em vez de derrubar a API inteira) e os testes unitários da lógica de tick (`lotes/lote-tick.spec.ts`, puro, sem Prisma). Falta rodar `prisma migrate dev` contra um banco real e testar end-to-end.

**Sobre o `.env` da API:** `main.ts` carrega `apps/api/.env` via `dotenv/config` — sem isso, `nest start`/`node dist/main.js` não leem o arquivo sozinhos (diferente do Prisma CLI, que carrega `.env` automaticamente). Se você já tinha criado `apps/api/.env` antes e via erro de `DATABASE_URL` mesmo assim, era esse o motivo — já corrigido.
